...
public class manager<E> {
    ...
    public String findUserIdx(String email) {
        //Checks if the directory is empty. If it is, there's nothing to search so return null.
        if (root == null) {
            System.out.println("The directory is empty");
            return null;
        }  
        //Creates a new stack that accepts the type searchTreeNode<String>
        Stack<searchTreeNode<String>> stack = new Stack<>(); 
        searchTreeNode<String> current = root; 
        //While current is not equals to null and stack is not empty
        while (current != null || !stack.isEmpty()) {
            //Adds the accounts (nodes) into the stack going through the left 
            while (current != null) { 
                stack.push(current); 
                current = current.left; 
            } 
            //Takes the top node out and checks it
            current = stack.pop(); 
            if (current.email.equals(email)) { 
                return current.userIdx; 
            } 
            //Moves to the next branch of the directory
    ...
    public void forPrint(searchTreeNode<String> root) {
        System.out.println(root.userIdx + ": " + root.firstName + " " + root.lastName);
    ...
    public void modify(String userIdx, String firstName, String lastName, String streetAddress, String city, 
